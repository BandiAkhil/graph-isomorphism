...
class Vertex(object):
    ...
    def __init__(self, graph: "Graph", n, label=None, color_num=None):
        """
    ...
    def __repr__(self):
        ...
        """
        return 'Vertex(label={}, #incident={}, color_num={})'.format(self.label, len(self._incidence), self.color_num)
...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        for _v in self.vertices:
            v = Vertex(g, _v.n, _v.label)
            v.original_graph = _v.graph
            vertices[_v] = v
            g.add_vertex(v)
        offset = len(g.vertices)
        for _v in other.vertices:
            v = Vertex(g, offset + _v.n, _v.label)
            v.original_graph = other
            vertices[_v] = v
            g.add_vertex(v)
        for _e in self.edges + other.edges:
            e = Edge(vertices[_e.tail], vertices[_e.head])
            g.add_edge(e)
        return g
